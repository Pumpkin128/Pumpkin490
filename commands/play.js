...
.setColor(COLOR);
    ...
    if (!args.length) {
      ...
      embed.setDescription(`**To Play A Music Type \`$play <url> or Music Name\`**`)
    ...
    if (!channel) {
      ...
      embed.setAuthor("You Need To Bee In A Voice Channel")
      return message.channel.send(embed);
    }
    ...
    if (urlcheck) {
      try {
        ...
        if (message.include === "copyright") {
          return message
            .reply("There Is Copytight Content In Video !")
            .catch(console.error);
        } else {
          console.error(error);
        }
    ...
    if (!serverQueue) {
      try {
        ...
        return message.channel
          .send({
            embed: {
              description: `You Need To Join
              Any Voice Channel: ${error}`,
              color: "#ff2050"
            }
          })
          .catch(console.error);
